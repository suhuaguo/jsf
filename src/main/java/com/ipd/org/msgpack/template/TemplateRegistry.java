...
import com.ipd.org.msgpack.MessagePackable;
import com.ipd.jsf.gd.error.JSFCodecException;
import com.ipd.org.msgpack.MessageTypeException;
import com.ipd.org.msgpack.template.builder.TemplateBuilder;
import com.ipd.org.msgpack.template.builder.TemplateBuilderChain;
...
public class TemplateRegistry {
    ...
    public synchronized void register(final Type targetType, final Template tmpl) {
        ...
        if (targetType instanceof ParameterizedType) {
            try {
                Class typeClass = (Class)((ParameterizedType) targetType).getRawType();
                Template temp = this.lookup(typeClass);
    ...
    private Template lookupGenericArrayTypeImpl(GenericArrayType genericArrayType) {
        ...
        if (dim <= 0) {
            throw new MessageTypeException(
                    String.format("fatal error: type= %s", genericArrayTypeName));
        } else if (dim > 1) {
        ...
        if(genericCompTypeName.indexOf("<") > -1){
            genericCompTypeName = genericCompTypeName.substring(0, genericCompTypeName.indexOf("<"));
        }
    ...
    private <T> Template<T> lookupInterfaceTypes(Class<T> targetClass) {
        ...
        for (Class<?> infType : infTypes) {
            ...
            if (tmpl != null) {
                if(!targetClass.isInterface() && tmpl.getClass().equals(AnyTemplate.class)) {
                    tmpl = null;
                    continue;
                }
                register(targetClass, tmpl);
    ...
    private synchronized Template buildAndRegister(TemplateBuilder builder,
        ...
        try {
            ...
            if(oldTmpl != null && !(oldTmpl instanceof TemplateReference)){
                return oldTmpl;
            }
    ...
    public Template getTemplateFromCache(Type targetType){
        ...
        try {
            template = this.lookup(type);
        } catch (Exception e) {
            template = null;
        }
        if(template == null){
            TemplateBuilder builder = chain.select(type, true);
            return this.buildAndRegister(builder, type, false, null);
        }
        if(template instanceof TemplateReference){
            TemplateReference tr = (TemplateReference)template;
            Template actualTemplate = tr.getActualTemplate();
            if(actualTemplate == null || actualTemplate instanceof TemplateReference){
                TemplateBuilder builder = chain.select(type, true);
                return this.buildAndRegister(builder, type, false, null);
            }
        }
    ...
    private void registryParameterizedType(ParameterizedType parameterizedType){
        ...
        for(Type type : types){
            if(type instanceof ParameterizedType){
                registryParameterizedType((ParameterizedType)type);
            }

            if(type instanceof GenericArrayType){
                registryGenericArrayType((GenericArrayType)type);
            }
            if(type instanceof Class){
                Class clazz = (Class)type;
                Template template = null;
                try {
                    template = this.lookup(clazz);
                } catch (Exception e) {
                    template = null;
                }
                if(template == null) {
                    TemplateBuilder builder = chain.select(clazz, true);
                    buildAndRegister(builder, clazz, false, null);
                }
            }
        }
        ...
        Template temp = null;
        try {
            temp = this.lookup(typeClass);
        } catch (MessageTypeException e) {
            temp = null;
        }
        if(temp == null || temp instanceof TemplateReference) {
            TemplateBuilder builder = chain.select(typeClass, true);
            buildAndRegister(builder, typeClass, false, null);
        }
    }

    private void registryGenericArrayType(GenericArrayType arrayType){
