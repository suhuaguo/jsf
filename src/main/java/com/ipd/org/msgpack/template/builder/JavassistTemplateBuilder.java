...
import com.ipd.org.msgpack.template.*;
    ...
    private Template<?>[] toTemplate(FieldEntry[] from) {
        ...
        for (int i = 0; i < from.length; ++i) {
            ...
            if (!e.isAvailable()) {
                ...
                } catch (MessageTypeException e1) {
                    tmpl= new TemplateReference(JSFMsgPack.getTemplateRegistry(), e.getGenericType());
                    registry.register(e.getGenericType(), tmpl);
                }
                tmpls[i] = tmpl;
            }
        }
