...
public abstract class BuildContext<T extends FieldEntry> {
     ...
     */
    private final static JSFLogger JSF_LOGGER = JSFLoggerFactory.getLogger(BuildContext.class);
    ...
    protected Template build(final String className) {
        try {
            ...
            if (code != null) {
                JSF_LOGGER.debug("builder: " + code);
            }
            throw new TemplateBuildException("Cannot compile "+className+"'s template : ", e);
        }
    }

    protected void reset(String className, boolean isWritten) {
        if(className.startsWith("java.")){
            className = "com.ipd.jsf." + className;
        }
    ...
    public static String primitiveWriteName(Class<?> type) {
        return "write";
    }

    public static String primitiveReadName(Class<?> type) {
