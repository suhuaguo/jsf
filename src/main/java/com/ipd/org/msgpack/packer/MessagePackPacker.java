...
//
package com.ipd.org.msgpack.packer;

import com.ipd.org.msgpack.io.Output;
...
import com.ipd.org.msgpack.io.StreamOutput;
...
public class MessagePackPacker extends AbstractPacker {
    ...
    public boolean addRef(Object object) throws IOException {
        if (!MsgpackUtil.openRefCheck(true)) {
            return false; // 关闭循环依赖检查特性
        }

        int ref = _refs.get(object);

        if (ref >= 0) {
            out.writeByte(JSFMsgPack.REF_CTRL);
            out.writeInt(ref);
            stack.reduceCount();
            return true;
        }
