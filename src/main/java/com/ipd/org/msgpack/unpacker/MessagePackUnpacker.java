...
//
package com.ipd.org.msgpack.unpacker;

import com.ipd.org.msgpack.type.ValueType;
...
import com.ipd.org.msgpack.io.BufferReferer;
import com.ipd.org.msgpack.io.Input;
import com.ipd.org.msgpack.io.StreamInput;
...
public class MessagePackUnpacker extends AbstractUnpacker {
    ...
    private final Vector _refs = new Vector();
    ...
    @Override
    public Object readRef(int index) throws IOException {
        return _refs.get(index);
    }

    @Override
    public int isRef() throws IOException {
        if (!MsgpackUtil.openRefCheck(false)) {
            return -1; // 关闭循环依赖检查特性
        }

        byte flag = this.getHeadByte();
        if(flag == JSFMsgPack.REF_CTRL){
            int index = in.getInt();
            in.advance();
            headByte = REQUIRE_TO_READ_HEAD;
            stack.reduceCount();
    ...
    @Override
    public void setRef(Object obj) throws IOException {
        if (!MsgpackUtil.openRefCheck(false)) {
            return; // 关闭循环依赖检查特性
        }
