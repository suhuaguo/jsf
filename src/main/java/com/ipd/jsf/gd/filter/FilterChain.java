...
import com.ipd.jsf.gd.config.ext.ExtensibleClass;
import com.ipd.jsf.gd.config.ext.ExtensionLoader;
import com.ipd.jsf.gd.config.ext.ExtensionLoaderFactory;
 ...
 * Title: 执行过滤器链<br>
 * <p/>
 * Description: 包含内置过滤器和用户自定义过滤器<br>
 * <p/>
 */
public class FilterChain {
        ...
        if (CommonUtils.isNotEmpty(filters)) {
            for (int i = filters.size() - 1; i >= 0; i--) {
                try {
                    ...
                    throw new InitErrorException("加载filter列表异常", e);
                }
            }
        }
    }
    ...
    public static FilterChain buildProviderChain(ProviderConfig providerConfig, Filter lastFilter) {
        ...
        if ( extensionLoader != null ){
            for (ExtensibleClass<Filter> extensibleClass : extensionLoader.getProviderSideAutoActives()){
                Filter filter = extensibleClass.getExtInstance();
    ...
    public static FilterChain buildConsumerChain(ConsumerConfig consumerConfig, Filter lastFilter) {
        ...
        ExtensionLoader<Filter> extensionLoader = ExtensionLoaderFactory.getExtensionLoader(Filter.class);
        if ( extensionLoader != null ){
            for (ExtensibleClass<Filter> extensibleClass : extensionLoader.getConsumerSideAutoActives()){
                Filter filter = extensibleClass.getExtInstance();
                if ( filter != null && filter instanceof AbstractFilter){
