...
import com.ipd.jsf.gd.monitor.ProviderStat;
 ...
 * Title: 连接保持器<br>
 * <p/>
 * Description: 视alive+dead+retry为一体，写操作要加锁，读随便读<br>
 * <p/>
 */
public class ConnectionHolder {
     ...
     * 失败的客户端(从来没连上过的）
     */
    private ConcurrentHashMap<Provider, ClientTransportConfig> deadConnections
            = new ConcurrentHashMap<Provider, ClientTransportConfig>();
    /**
     * 失败待重试的客户端列表（连上后断开的）
     */
     ...
     */
    public ConcurrentHashMap<Provider, ClientTransportConfig> getDeadConnections() {
     ...
     *         the config
     */
    protected void addDead(Provider provider, ClientTransportConfig config) {
    ...
    private void doReconnect() {
        ...
        for (Map.Entry<Provider, ClientTransportConfig> entry : getDeadConnections()
                .entrySet()) {
            ...
            ClientTransportConfig config = entry.getValue();
            ...
            try {
                ...
                if (doubleCheck(interfaceId, provider, transport)) {
                    ...
                    LOGGER.debug("Connect to {} provider:{} failure !", interfaceId, provider);
    ...
    private void sendHeartbeatToProvider(Provider provider, ClientTransport transport, boolean isAliveProvider) {
        ...
        if (protocolType != Constants.ProtocolType.jsf && protocolType != Constants.ProtocolType.dubbo) {
            return; // 指定协议才发
        }
        if (!transport.isOpen()) {
        ...
        if (protocolType == Constants.ProtocolType.dubbo) { // dubbo的发hessian
            message.getMsgHeader().setCodecType(Constants.CodecType.hessian.value());
        }
        ...
        if (!ok && exception != null) { // 连续2次心跳异常
            ...
            String monitor = consumerConfig.getParameter(Constants.HIDDEN_KEY_MONITOR);
    ...
    public ProviderCheckedInfo checkProvider(Provider provider) {
        ...
        if (provider.getProtocolType() == Constants.ProtocolType.jsf) {
