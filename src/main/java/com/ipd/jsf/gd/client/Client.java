...
import com.ipd.jsf.gd.transport.ClientTransportConfig;
 ...
 * Title: 客户端主要代码<br>
 * <p/>
 * Description: 实现了客户端下长连接的维护，长连接选择，同步异步调用等公共方法，子类需要实现具体的规则<br>
 * <p/>
 */
public abstract class Client {
    ...
    public Client(ConsumerConfig<?> consumerConfig) {
        ...
        loadbalance.setConsumerConfig(consumerConfig);
    ...
    private List<Router> initRouterByRule() {
        ...
        boolean open = CommonUtils.isTrue(JSFContext.getInterfaceVal(interfaceId, Constants.SETTING_ROUTER_OPEN, "true"));
        if (open) {
            String routerRule = JSFContext.getInterfaceVal(interfaceId, Constants.SETTING_ROUTER_RULE, null);
    ...
    private void initConnections() {
        if (destroyed) { // 已销毁
            throw new RpcException("[JSF-22001]Client has been destroyed!");
        }
        if (inited) { // 已初始化
            return;
        }
    ...
    protected List<Provider> buildProviderList() {
        ...
        if (StringUtils.isNotEmpty(url)) { // 如果走直连
            ...
            for (int i = 0; i < providerStrs.length; i++) {
                ...
                if(!CommonUtils.isFalse(consumerConfig.getParameter(Constants.HIDDEN_KEY_WARNNING))) {
    ...
    protected ResponseMessage sendMsg0(Connection connection, RequestMessage msg) {
        ...
        try {
            ...
            if (async || CommonUtils.isTrue(genericAsync)) {
                ...
                Object methodResponseListenerObj = RpcContext.getContext().getAttachment(Constants.INTERNAL_KEY_ONRETURN);
                ...
                final ResponseListener genericAsynReturn = (ResponseListener)
                        invocation.getAttachment(Constants.INTERNAL_KEY_ONRETURN);
                if (genericAsynReturn != null) {
                    invocation.addAttachment(Constants.INTERNAL_KEY_ONRETURN, null);
                    if (resultListener == null) {
                        resultListener = new AsyncResultListener();
                    }
                ...
                // RpcContext.getContext().setAttachment(Constants.CONFIG_KEY_ASYNC, true);
            }
     ...
     *         请求对象
     */
    private void checkProviderVersion(Provider provider, RequestMessage request) {
        int version = provider.getJsfVersion();
        if (version >= 1500) {
            // 服务端版本号供本地序列化使用
            RpcContext.getContext().setAttachment(Constants.HIDDEN_KEY_DST_JSF_VERSION, (short) version);
            // head增加客户端版本号供服务端用
            request.getMsgHeader().addHeadKey(Constants.HeadKey.jsfVersion, (short) Constants.JSF_VERSION);
            Invocation invocation = request.getInvocationBody();
            if (!CommonUtils.isTrue((Boolean) invocation.getAttachment(Constants.CONFIG_KEY_GENERIC))
                    && provider.openInvocationOptimizing()) { // 是否开启invocation优化
                invocation.setIfaceId(JSFContext.getIfaceIdByClassName(consumerConfig.getInterfaceId()));
            } else {
     ...
     *         没有可用的服务端
     */
    protected Connection select(RequestMessage message, List<Provider> invokedProviders) throws NoAliveProviderException {
        // 粘滞连接，当前连接可用
        ...
        do {
            // 再进行负载均衡筛选
            Provider provider = loadbalance.select(invocation, providers);
     ...
     *         请求对象
     */
    private void checkAlias(Provider provider, RequestMessage message) {
        Invocation invocation = message.getInvocationBody();
        String pAlias = provider.getAlias();
        ...
        Constants.CodecType ct = provider.getCodecType();
        if (ct != null) {
    ...
    protected void closeTransports() {
        ...
        if (count > 0) { // 有正在调用的请求
            ...
            while (countOfInvoke.get() > 0 && JSFContext.systemClock.now() - start < timeout) { // 等待返回结果
                try {
                    Thread.sleep(10);
                } catch (InterruptedException e) {
                }
