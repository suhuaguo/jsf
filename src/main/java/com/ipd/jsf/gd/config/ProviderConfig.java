...
import com.ipd.jsf.gd.server.ProviderProxyInvoker;
 ...
 * Title: 服务发布端配置<br>
 * <p/>
 * Description: 对应jsf:service，对应一个接口的服务发布者<br>
 * <p/>
 */
public class ProviderConfig<T> extends AbstractInterfaceConfig<T> implements Serializable {
     ...
     */
    private static final long serialVersionUID = 1966427902902394280L;
     ...
     * @throws InitErrorException the init error exception
     */
    private synchronized void doExport() throws InitErrorException {
        if (exported) {
            return;
        }
        String key = buildKey();
        ...
        if (StringUtils.isBlank(alias)) {
            throw new InitErrorException("[JSF-21200]Value of \"alias\" is not specified in provider" +
                    " config with key " + key + " !");
        }
        // 检查注入的ref是否接口实现类
        if (!getProxyClass().isInstance(ref)) {
            throw new IllegalConfigureException(21201, "provider.ref", ref.getClass().getName(),
        ...
        if (CommonUtils.isEmpty(serverConfigs)) {
            throw new InitErrorException("[JSF-21202]Value of \"server\" is not specified in provider" +
                    " config with key " + key + " !");
        }
        ...
        if (EXPORTED_KEYS.contains(key)) {
            ...
            throw new InitErrorException("[JSF-21203]Duplicate provider config with key " + key + " has been exported!");
        }
...
//        register();
        ...
        JSFContext.cacheProviderConfig(this);
        exported = true;
    }
    ...
    private void checkOverloadingMethod(Class<?> itfClass) {
        ...
        for (Method method : itfClass.getMethods()) {
            ...
            if (methodsLimit.containsKey(methodName)) {
                ...
                throw new InitErrorException("[JSF-21205]Method with same name \"" + itfClass.getName() + "."
    ...
    public synchronized void unexport() {
        ...
        if (serverConfigs != null) {
            for (ServerConfig serverConfig : serverConfigs) {
                ...
                try {
                    server.unregisterProcessor(this);
                } catch (Exception e) {
        ...
        JSFContext.invalidateProviderConfig(this);
        ...
        try {
            if (StringUtils.isNotBlank(interfaceId)) {
                ...
                if (!proxyClass.isInterface()) {
                    throw new IllegalConfigureException(21206, "service.interfaceId",
                            interfaceId,
                            "interfaceId must set interface class, not implement class");
                }
            } else {
                throw new IllegalConfigureException(21207, "service.interfaceId",
                        "null", "interfaceId must be not null");
            }
        } catch (ClassNotFoundException t) {
            throw new InitErrorException(t.getMessage(), t);
        }
        return proxyClass;
    }
    ...
    protected void register() {
        if (isRegister()) {
            ...
            if (registryConfigs != null) {
                ...
                super.setParameter(Constants.CONFIG_KEY_CROSSLANG, crossLang + "");
                for (RegistryConfig registryConfig : registryConfigs) {
                ...
                super.setParameter(Constants.CONFIG_KEY_CROSSLANG, null);
            }
        }
    }
    ...
    private class ProviderAttributeListener implements ConfigListener {
        ...
        @Override
        public synchronized void providerAttrUpdated(Map newValueMap) {
            ...
            if (servers != null && !servers.isEmpty()) {
                ...
                for (ServerConfig server : servers) {
                    ...
                    sb.append(server.getProtocol()).append("://").append(server.getHost())
                            ...
                            .append(getInterfaceId()).append("?alias=").append(getAlias())
                            ...
                            .append(getKeyPairs(Constants.CONFIG_KEY_SAFVERSION, Constants.DEFAULT_SAF_VERSION))
                            .append(getKeyPairs(Constants.CONFIG_KEY_JSFVERSION, Constants.JSF_VERSION));
