...
import com.ipd.fastjson.asm.*;
import com.ipd.fastjson.parser.*;
import com.ipd.fastjson.util.ASMClassLoader;
import com.ipd.fastjson.util.ASMUtils;
import com.ipd.fastjson.util.DeserializeBeanInfo;
import com.ipd.fastjson.util.FieldInfo;
import com.ipd.fastjson.asm.ASMException;
import com.ipd.fastjson.asm.ClassWriter;
import com.ipd.fastjson.asm.FieldVisitor;
import com.ipd.fastjson.asm.Label;
import com.ipd.fastjson.asm.MethodVisitor;
import com.ipd.fastjson.asm.Opcodes;
...
import com.ipd.fastjson.parser.JSONLexerBase;
...
import com.ipd.fastjson.parser.SymbolTable;
import com.ipd.fastjson.util.ASMClassLoader;
import com.ipd.fastjson.util.ASMUtils;
import com.ipd.fastjson.util.DeserializeBeanInfo;
import com.ipd.fastjson.util.FieldInfo;

import static com.ipd.fastjson.util.ASMUtils.getDesc;
...
public class ASMDeserializerFactory implements Opcodes {
    ...
    public ObjectDeserializer createJavaBeanDeserializer(ParserConfig config, Class<?> clazz, Type type)
        ...
        cw.visit(V1_5, ACC_PUBLIC + ACC_SUPER, className, ASMUtils.getType(ASMJavaBeanDeserializer.class), null);
    ...
    void _deserialzeArrayMapping(ClassWriter cw, Context context) {
        MethodVisitor mw = cw.visitMethod(ACC_PUBLIC, "deserialzeArrayMapping",
                                          "(" + ASMUtils.getDesc(DefaultJSONParser.class) + ASMUtils.getDesc(Type.class)
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == byte.class //
                ...
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanInt", "(C)I");
                mw.visitVarInsn(ISTORE, context.var(fieldInfo.getName() + "_asm"));
            } else if (fieldClass == long.class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(BIPUSH, seperator);
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanLong", "(C)J");
                mw.visitVarInsn(LSTORE, context.var(fieldInfo.getName() + "_asm", 2));
            } else if (fieldClass == boolean.class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(BIPUSH, seperator);
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanBoolean", "(C)Z");
                mw.visitVarInsn(ISTORE, context.var(fieldInfo.getName() + "_asm"));
            } else if (fieldClass == float.class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(BIPUSH, seperator);
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanFloat", "(C)F");
                mw.visitVarInsn(FSTORE, context.var(fieldInfo.getName() + "_asm"));
            } else if (fieldClass == double.class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(BIPUSH, seperator);
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanDouble", "(C)D");
                mw.visitVarInsn(DSTORE, context.var(fieldInfo.getName() + "_asm", 2));
            } else if (fieldClass == char.class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(BIPUSH, seperator);
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanString", "(C)Ljava/lang/String;");
                mw.visitInsn(ICONST_0);
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(String.class), "charAt", "(I)C");
                mw.visitVarInsn(ISTORE, context.var(fieldInfo.getName() + "_asm"));
                ...
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitLdcInsn(com.ipd.fastjson.asm.Type.getType(ASMUtils.getDesc(fieldClass)));
                mw.visitVarInsn(ALOAD, 1);
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.class), "getSymbolTable",
                                   "()" + ASMUtils.getDesc(SymbolTable.class));
                mw.visitVarInsn(BIPUSH, seperator);
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanEnum",
                                   "(Ljava/lang/Class;" + ASMUtils.getDesc(SymbolTable.class) + "C)Ljava/lang/Enum;");
                mw.visitTypeInsn(CHECKCAST, ASMUtils.getType(fieldClass)); // cast
                mw.visitVarInsn(ASTORE, context.var(fieldInfo.getName() + "_asm"));
                ...
                if (itemClass == String.class) {
                    ...
                    mw.visitLdcInsn(com.ipd.fastjson.asm.Type.getType(ASMUtils.getDesc(fieldClass)));
                    mw.visitVarInsn(BIPUSH, seperator);
                    mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanStringArray",
                    ...
                    mw.visitVarInsn(ALOAD, 1);
                    if (i == 0) {
                        mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(JSONToken.class), "LBRACKET", "I");
                    } else {
                        mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(JSONToken.class), "COMMA", "I");
                    }
                    mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(JSONToken.class), "LBRACKET", "I");
                    mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.class), "accept", "(II)V");
                    ...
                    mw.visitVarInsn(ALOAD, 1);
                    mw.visitLdcInsn(com.ipd.fastjson.asm.Type.getType(ASMUtils.getDesc(itemClass)));
                    mw.visitVarInsn(ALOAD, 3);
                    mw.visitMethodInsn(INVOKESTATIC, ASMUtils.getType(ASMUtils.class), "parseArray",
                ...
                if (!last) {
                    ...
                    mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(JSONToken.class), "RBRACKET", "I");
                    mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(JSONToken.class), "EOF", "I");
                }
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.class), "accept", "(II)V");
                continue;
            }
        ...
        mw.visitVarInsn(ALOAD, context.var("lexer"));
        mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(JSONToken.class), "COMMA", "I");
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "nextToken", "(I)V");
    ...
    void _deserialze(ClassWriter cw, Context context) {
        ...
        MethodVisitor mw = cw.visitMethod(ACC_PUBLIC, "deserialze", "(" + ASMUtils.getDesc(DefaultJSONParser.class)
                                                                    + ASMUtils.getDesc(Type.class)
...

        {
            ...
            mw.visitVarInsn(ALOAD, context.var("lexer"));
            mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "token", "()I");
            mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(JSONToken.class), "LBRACKET", "I");
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanType", "(Ljava/lang/String;)I");

        mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(JSONLexerBase.class), "NOT_MATCH", "I");
        mw.visitJumpInsn(IF_ICMPEQ, super_);

        mw.visitVarInsn(ALOAD, 1); // parser
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.class), "getContext",
                           "()Lcom/ipd/fastjson/parser/ParseContext;");
...

        {
            ...
            mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.class), "setContext",
        ...
        mw.visitVarInsn(ALOAD, context.var("lexer"));
        mw.visitFieldInsn(GETFIELD, ASMUtils.getType(JSONLexerBase.class), "matchStat", "I");
        mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(JSONLexerBase.class), "END", "I");
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == boolean.class //
                ...
                if (fieldClass == String.class) {
                    ...
                    mw.visitVarInsn(ALOAD, context.var("lexer"));
                    mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "stringDefaultValue",
                                       "()Ljava/lang/String;");
                } else {
        ...
        for (int i = 0; i < fieldListSize; ++i) {
            ...
            if (fieldClass == boolean.class) {
                ...
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanFieldBoolean", "([C)Z");
                mw.visitVarInsn(ISTORE, context.var(fieldInfo.getName() + "_asm"));
            } else if (fieldClass == byte.class) {
                mw.visitVarInsn(ALOAD, context.var("lexer"));
                mw.visitVarInsn(ALOAD, 0);
                mw.visitFieldInsn(GETFIELD, context.getClassName(), fieldInfo.getName() + "_asm_prefix__", "[C");
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanFieldInt", "([C)I");
                mw.visitVarInsn(ISTORE, context.var(fieldInfo.getName() + "_asm"));
                ...
                mw.visitVarInsn(ALOAD, 0);
                mw.visitFieldInsn(GETFIELD, context.getClassName(), fieldInfo.getName() + "_asm_prefix__", "[C");
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanFieldLong", "([C)J");
                mw.visitVarInsn(LSTORE, context.var(fieldInfo.getName() + "_asm", 2));
                ...
                mw.visitVarInsn(ALOAD, 0);
                mw.visitFieldInsn(GETFIELD, context.getClassName(), fieldInfo.getName() + "_asm_prefix__", "[C");
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanFieldFloat", "([C)F");
                mw.visitVarInsn(FSTORE, context.var(fieldInfo.getName() + "_asm"));
                ...
                mw.visitVarInsn(ALOAD, 0);
                mw.visitFieldInsn(GETFIELD, context.getClassName(), fieldInfo.getName() + "_asm_prefix__", "[C");
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanFieldDouble", "([C)D");
                mw.visitVarInsn(DSTORE, context.var(fieldInfo.getName() + "_asm", 2));
                ...
                mw.visitVarInsn(ALOAD, 0);
                mw.visitFieldInsn(GETFIELD, context.getClassName(), fieldInfo.getName() + "_asm_prefix__", "[C");
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanFieldString",
                                   "([C)Ljava/lang/String;");
                mw.visitVarInsn(ASTORE, context.var(fieldInfo.getName() + "_asm"));
                ...
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanFieldSymbol",
                                   "([C" + ASMUtils.getDesc(SymbolTable.class) + ")Ljava/lang/String;");
                mw.visitInsn(DUP);
                ...
                mw.visitMethodInsn(INVOKESTATIC, ASMUtils.getType(fieldClass), "valueOf", "(Ljava/lang/String;)"
                ...
                if (itemClass == String.class) {
                    mw.visitLdcInsn(com.ipd.fastjson.asm.Type.getType(ASMUtils.getDesc(fieldClass))); // cast
                    mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "scanFieldStringArray",
        ...
        mw.visitVarInsn(ALOAD, 3);
        mw.visitVarInsn(ALOAD, context.var("instance"));
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(ASMJavaBeanDeserializer.class), "parseRest",
                           "(" + ASMUtils.getDesc(DefaultJSONParser.class) + ASMUtils.getDesc(Type.class)
                                   + "Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;");
        mw.visitTypeInsn(CHECKCAST, ASMUtils.getType(context.getClazz())); // cast
        mw.visitInsn(ARETURN);
        ...
        mw.visitVarInsn(ALOAD, 2);
        mw.visitVarInsn(ALOAD, 3);
        mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.getType(ASMJavaBeanDeserializer.class), "deserialze",
                           "(" + ASMUtils.getDesc(DefaultJSONParser.class) + ASMUtils.getDesc(Type.class)
                                   + "Ljava/lang/Object;)Ljava/lang/Object;");
        mw.visitInsn(ARETURN);
    ...
    private void _isEnable(Context context, MethodVisitor mw, Feature feature) {
        mw.visitVarInsn(ALOAD, context.var("lexer"));
        mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(Feature.class), feature.name(), "L" + ASMUtils.getType(Feature.class) + ";");
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "isEnabled", "(" + "L" + ASMUtils.getType(Feature.class)
                                                                                     + ";" + ")Z");
    }

    private void defineVarLexer(Context context, MethodVisitor mw) {
        mw.visitVarInsn(ALOAD, 1);
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.class), "getLexer", "()" + ASMUtils.getDesc(JSONLexer.class));
        mw.visitTypeInsn(CHECKCAST, ASMUtils.getType(JSONLexerBase.class)); // cast
    ...
    private void _createInstance(Context context, MethodVisitor mw) {
        ...
        if (Modifier.isPublic(defaultConstructor.getModifiers())) {
            mw.visitTypeInsn(NEW, ASMUtils.getType(context.getClazz()));
            mw.visitInsn(DUP);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.getType(context.getClazz()), "<init>", "()V");

            mw.visitVarInsn(ASTORE, context.var("instance"));
        } else {
            mw.visitVarInsn(ALOAD, 0);
            mw.visitVarInsn(ALOAD, 1);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.getType(ASMJavaBeanDeserializer.class), "createInstance",
                               "(" + ASMUtils.getDesc(DefaultJSONParser.class) + ")Ljava/lang/Object;");
            mw.visitTypeInsn(CHECKCAST, ASMUtils.getType(context.getClazz())); // cast
            mw.visitVarInsn(ASTORE, context.var("instance"));
        }
    }
    ...
    private void _loadAndSet(Context context, MethodVisitor mw, FieldInfo fieldInfo) {
        ...
        if (fieldClass == boolean.class) {
            ...
            if (fieldInfo.getMethod() != null) {
                mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(context.getClazz()), fieldInfo.getMethod().getName(),
                                   ASMUtils.getDesc(fieldInfo.getMethod()));
                if (!fieldInfo.getMethod().getReturnType().equals(Void.TYPE)) {
                    mw.visitInsn(POP);
                }
            } else {
                mw.visitFieldInsn(PUTFIELD, ASMUtils.getType(fieldInfo.getDeclaringClass()), fieldInfo.getField().getName(),
    ...
    private void _set(Context context, MethodVisitor mw, FieldInfo fieldInfo) {
        if (fieldInfo.getMethod() != null) {
            mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(fieldInfo.getDeclaringClass()), fieldInfo.getMethod().getName(),
                               ASMUtils.getDesc(fieldInfo.getMethod()));
    ...
    private void _setContext(Context context, MethodVisitor mw) {
        ...
        mw.visitVarInsn(ALOAD, context.var("childContext"));
        mw.visitVarInsn(ALOAD, context.var("instance"));
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(ParseContext.class), "setObject", "(Ljava/lang/Object;)V");
    ...
    private void _deserialize_endCheck(Context context, MethodVisitor mw, Label reset_) {
        ...
        mw.visitVarInsn(ALOAD, context.var("lexer"));
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "token", "()I");
        mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(JSONToken.class), "RBRACE", "I");
        mw.visitJumpInsn(IF_ICMPNE, reset_);
    ...
    private void _deserialze_list_obj(Context context, MethodVisitor mw, Label reset_, FieldInfo fieldInfo,
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "matchField", "([C)Z");
        ...
        mw.visitVarInsn(ALOAD, context.var("lexer"));
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JSONLexerBase.class), "token", "()I");
        mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(JSONToken.class), "NULL", "I");
        ...
        mw.visitMethodInsn(INVOKEINTERFACE, ASMUtils.getType(ObjectDeserializer.class), "getFastMatchToken", "()I");
        ...
        mw.visitFieldInsn(GETFIELD, context.getClassName(), fieldInfo.getName() + "_asm_list_item_deser__",
                          ASMUtils.getDesc(ObjectDeserializer.class));
        mw.visitVarInsn(ALOAD, 1);
        mw.visitLdcInsn(com.ipd.fastjson.asm.Type.getType(ASMUtils.getDesc(itemType)));
        ...
        mw.visitMethodInsn(INVOKEINTERFACE, ASMUtils.getType(ObjectDeserializer.class), "deserialze",
                           "(" + ASMUtils.getDesc(DefaultJSONParser.class)
        ...
        if (fieldClass.isInterface()) {
            mw.visitMethodInsn(INVOKEINTERFACE, ASMUtils.getType(fieldClass), "add", "(Ljava/lang/Object;)Z");
        } else {
            mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(fieldClass), "add", "(Ljava/lang/Object;)Z");
        }
        mw.visitInsn(POP);

        mw.visitVarInsn(ALOAD, 1);
        mw.visitVarInsn(ALOAD, context.var(fieldInfo.getName() + "_asm"));
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.class), "checkListResolve",
    ...
    private void _getCollectionFieldItemDeser(Context context, MethodVisitor mw, FieldInfo fieldInfo, Class<?> itemType) {
        ...
        mw.visitVarInsn(ALOAD, 1);
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.class), "getConfig", "()"
                                                                                         + ASMUtils.getDesc(ParserConfig.class));
        mw.visitLdcInsn(com.ipd.fastjson.asm.Type.getType(ASMUtils.getDesc(itemType)));
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(ParserConfig.class), "getDeserializer",
                           "(" + ASMUtils.getDesc(Type.class) + ")" + ASMUtils.getDesc(ObjectDeserializer.class));
    ...
    private void _newCollection(MethodVisitor mw, Class<?> fieldClass) {
        if (fieldClass.isAssignableFrom(ArrayList.class)) {
            mw.visitTypeInsn(NEW, ASMUtils.getType(ArrayList.class));
            mw.visitInsn(DUP);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.getType(ArrayList.class), "<init>", "()V");
        } else if (fieldClass.isAssignableFrom(LinkedList.class)) {
            mw.visitTypeInsn(NEW, ASMUtils.getType(LinkedList.class));
            mw.visitInsn(DUP);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.getType(LinkedList.class), "<init>", "()V");
        } else if (fieldClass.isAssignableFrom(HashSet.class)) {
            mw.visitTypeInsn(NEW, ASMUtils.getType(HashSet.class));
            mw.visitInsn(DUP);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.getType(HashSet.class), "<init>", "()V");
        } else if (fieldClass.isAssignableFrom(TreeSet.class)) {
            mw.visitTypeInsn(NEW, ASMUtils.getType(TreeSet.class));
            mw.visitInsn(DUP);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.getType(TreeSet.class), "<init>", "()V");
        } else {
            mw.visitTypeInsn(NEW, ASMUtils.getType(fieldClass));
            mw.visitInsn(DUP);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.getType(fieldClass), "<init>", "()V");
        }
        mw.visitTypeInsn(CHECKCAST, ASMUtils.getType(fieldClass)); // cast
    }
    ...
    private void _deserialze_obj(Context context, MethodVisitor mw, Label reset_, FieldInfo fieldInfo,
        ...
        mw.visitVarInsn(ALOAD, 1);
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.class), "getResolveStatus", "()I");
        mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(DefaultJSONParser.class), "NeedToResolve", "I");
        ...
        mw.visitVarInsn(ALOAD, 1);
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.class), "getLastResolveTask",
                           "()" + ASMUtils.getDesc(DefaultJSONParser.ResolveTask.class));
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.class), "getContext", "()"
                                                                                          + ASMUtils.getDesc(ParseContext.class));
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.ResolveTask.class), "setOwnerContext", "("
                                                                                         + ASMUtils.getDesc(ParseContext.class)
                                                                                         + ")V");
        ...
        mw.visitVarInsn(ALOAD, 0);
        mw.visitLdcInsn(fieldInfo.getName());
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(ASMJavaBeanDeserializer.class), "getFieldDeserializer",
                           "(Ljava/lang/String;)" + ASMUtils.getDesc(FieldDeserializer.class));
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.ResolveTask.class), "setFieldDeserializer",
                           "(" + ASMUtils.getDesc(FieldDeserializer.class) + ")V");

        mw.visitVarInsn(ALOAD, 1);
        mw.visitFieldInsn(GETSTATIC, ASMUtils.getType(DefaultJSONParser.class), "NONE", "I");
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(DefaultJSONParser.class), "setResolveStatus", "(I)V");
    ...
    private void _deserObject(Context context, MethodVisitor mw, FieldInfo fieldInfo, Class<?> fieldClass) {
        ...
        if (fieldInfo.getFieldType() instanceof Class) {
            mw.visitLdcInsn(com.ipd.fastjson.asm.Type.getType(ASMUtils.getDesc(fieldInfo.getFieldClass())));
        } else {
            mw.visitVarInsn(ALOAD, 0);
            mw.visitLdcInsn(fieldInfo.getName());
            mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(ASMJavaBeanDeserializer.class), "getFieldType",
    ...
    public FieldDeserializer createStringFieldDeserializer(ParserConfig mapping, Class<?> clazz, FieldInfo fieldInfo)
        ...
        cw.visit(V1_5, ACC_PUBLIC + ACC_SUPER, className, ASMUtils.getType(superClass), null);

        {
            MethodVisitor mw = cw.visitMethod(ACC_PUBLIC, "<init>", "(" + ASMUtils.getDesc(ParserConfig.class)
                                                                    + ASMUtils.getDesc(Class.class) + ASMUtils.getDesc(FieldInfo.class)
                                                                    + ")V", null, null);
            mw.visitVarInsn(ALOAD, 0);
            mw.visitVarInsn(ALOAD, 1);
            mw.visitVarInsn(ALOAD, 2);
            mw.visitVarInsn(ALOAD, 3);
            mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.getType(superClass), "<init>", "(" + ASMUtils.getDesc(ParserConfig.class)
                                                                             + ASMUtils.getDesc(Class.class)
                                                                             + ASMUtils.getDesc(FieldInfo.class) + ")V");
        ...
        if (method != null) {
            if (fieldClass == int.class) {
                MethodVisitor mw = cw.visitMethod(ACC_PUBLIC, "setValue", "(" + ASMUtils.getDesc(Object.class) + "I)V", null,
                                                  null);
                mw.visitVarInsn(ALOAD, 1);
                mw.visitTypeInsn(CHECKCAST, ASMUtils.getType(method.getDeclaringClass())); // cast
                ...
                MethodVisitor mw = cw.visitMethod(ACC_PUBLIC, "setValue", "(" + ASMUtils.getDesc(Object.class) + "J)V", null,
                                                  null);
                mw.visitVarInsn(ALOAD, 1);
                mw.visitTypeInsn(CHECKCAST, ASMUtils.getType(method.getDeclaringClass())); // cast
                ...
                MethodVisitor mw = cw.visitMethod(ACC_PUBLIC, "setValue", "(" + ASMUtils.getDesc(Object.class)
                                                                          + ASMUtils.getDesc(Object.class) + ")V", null, null);
                mw.visitVarInsn(ALOAD, 1);
                mw.visitTypeInsn(CHECKCAST, ASMUtils.getType(method.getDeclaringClass())); // cast
                mw.visitVarInsn(ALOAD, 2);
                mw.visitTypeInsn(CHECKCAST, ASMUtils.getType(fieldClass)); // cast
    ...
    private void _init(ClassWriter cw, Context context) {
        ...
        MethodVisitor mw = cw.visitMethod(ACC_PUBLIC, "<init>", "(" + ASMUtils.getDesc(ParserConfig.class)
                                                                + ASMUtils.getDesc(Class.class) + ")V", null, null);
        mw.visitVarInsn(ALOAD, 0);
        mw.visitVarInsn(ALOAD, 1);
        mw.visitVarInsn(ALOAD, 2);
        mw.visitMethodInsn(INVOKESPECIAL, ASMUtils.getType(ASMJavaBeanDeserializer.class), "<init>",
                           "(" + ASMUtils.getDesc(ParserConfig.class) + ASMUtils.getDesc(Class.class) + ")V");

        mw.visitVarInsn(ALOAD, 0);
        mw.visitFieldInsn(GETFIELD, ASMUtils.getType(ASMJavaBeanDeserializer.class), "serializer",
                          ASMUtils.getDesc(ASMJavaBeanDeserializer.InnerJavaBeanDeserializer.class));
        mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(JavaBeanDeserializer.class), "getFieldDeserializerMap",
                           "()" + ASMUtils.getDesc(Map.class));
        mw.visitInsn(POP);
        ...
        for (int i = 0, size = context.getFieldInfoList().size(); i < size; ++i) {
            ...
            mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.getType(String.class), "toCharArray", "()" + ASMUtils.getDesc(char[].class));
    ...
    private void _createInstance(ClassWriter cw, Context context) {
        MethodVisitor mw = cw.visitMethod(ACC_PUBLIC, "createInstance", "(" + ASMUtils.getDesc(DefaultJSONParser.class)
                                                                        + ASMUtils.getDesc(Type.class) + ")Ljava/lang/Object;",
                                          null, null);
